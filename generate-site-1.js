...
class SiteGenerator {
    ...
    generateSite() {
        ...
        // No division/district/tehsil pages - only depots
    ...
    generateHomepage() {
        console.log('📄 Generating homepage with cascading dropdowns...');
...
<body>
    ...
    <script>
        ...
        console.log('   ✓ Homepage generated with cascading dropdowns and depot grid');
    }
...
class BusTimetableApp {
    ...
    initBlogLinks() {
        setTimeout(() => {
            if (typeof window.handleBlogLinkClick !== 'undefined') {
                document.querySelectorAll('.blog-link[data-constant]').forEach(link => {
                    link.addEventListener('click', window.handleBlogLinkClick);
                });
    ...
    filterDepotCards() {
        ...
        cards.forEach(card => {
            ...
            const distId = card.dataset.districtId;
            const tehId = card.dataset.tehsilId;
            const depotId = card.dataset.depotId;

            let match = true;
            if (divisionVal && divId !== divisionVal) match = false;
            if (match && districtVal && distId !== districtVal) match = false;
            if (match && tehsilVal && tehId !== tehsilVal) match = false;
            if (match && depotVal && depotId !== depotVal) match = false;
        ...
        const emptyState = document.getElementById('emptyDepotsState');
    ...
    updateVerticalAlphabetVisibility() {
        const visibleCards = Array.from(document.querySelectorAll('#depotsGrid .rectangular-card')).filter(c => c.style.display !== 'none');
        const lettersPresent = new Set(visibleCards.map(c => c.querySelector('h3')?.innerText.charAt(0).toUpperCase()).filter(l => l));
        document.querySelectorAll('.alphabet-vertical-btn').forEach(btn => {
            const letter = btn.dataset.letter;
            if (lettersPresent.has(letter)) btn.classList.remove('disabled');
    ...
    initDropdowns() {
        ...
        const tehsilSelect = document.getElementById('tehsilFilter');
        ...
        const updateDistrict = () => {
            ...
            const filtered = selectedDiv ? window.DISTRICTS_DATA.filter(d => d.division_id === selectedDiv) : window.DISTRICTS_DATA;
            filtered.forEach(d => {
        ...
        const updateTehsil = () => {
            ...
            if (selectedDist) filtered = filtered.filter(t => t.district_id === selectedDist);
            else if (selectedDiv) filtered = filtered.filter(t => {
                const dist = window.DISTRICTS_DATA.find(d => d.id === t.district_id);
                return dist && dist.division_id === selectedDiv;
            });
            filtered.forEach(t => {
                const opt = document.createElement('option');
                opt.value = t.id;
        ...
        const updateDepot = () => {
            const selectedDiv = divisionSelect.value;
            const selectedDist = districtSelect.value;
            const selectedTeh = tehsilSelect.value;
            depotSelect.innerHTML = '<option value="">All Depots</option>';
            let filtered = window.DEPOTS_DATA;
            if (selectedTeh) filtered = filtered.filter(d => d.tehsil_id === selectedTeh);
            else if (selectedDist) filtered = filtered.filter(d => {
                const teh = window.TEHSILS_DATA.find(t => t.id === d.tehsil_id);
                return teh && teh.district_id === selectedDist;
            });
            else if (selectedDiv) filtered = filtered.filter(d => {
                const teh = window.TEHSILS_DATA.find(t => t.id === d.tehsil_id);
                if (!teh) return false;
                const dist = window.DISTRICTS_DATA.find(ds => ds.id === teh.district_id);
                return dist && dist.division_id === selectedDiv;
            });
            filtered.forEach(d => {
                const opt = document.createElement('option');
        ...
        divisionSelect.addEventListener('change', () => { updateDistrict(); updateTehsil(); updateDepot(); });
        districtSelect.addEventListener('change', () => { updateTehsil(); updateDepot(); });
        tehsilSelect.addEventListener('change', () => { updateDepot(); });
        depotSelect.addEventListener('change', () => this.filterDepotCards());

        // Populate divisions
        window.DIVISIONS_DATA.forEach(d => {
            const opt = document.createElement('option');
            opt.value = d.id;
            opt.textContent = d.name;
    ...
    highlightAlphabetButton(clickedButton) {
        document.querySelectorAll('.alphabet-vertical-btn').forEach(btn => btn.classList.remove('active'));
    ...
    initScrollHighlighting() {
        ...
        window.addEventListener('scroll', () => {
            ...
            scrollTimeout = setTimeout(() => this.updateActiveAlphabet(), 50);
        });
    ...
    updateActiveAlphabet() {
        ...
        for (let card of cards) {
            ...
            const dist = Math.abs(scrollPosition - cardTop);
        ...
        if (closestCard) {
            const title = closestCard.querySelector('h3')?.innerText;
            if (title) {
    ...
    applyFilters() {
        ...
        sections.forEach(section => {
            ...
            if (busCount) busCount.textContent = \`\${visibleBubbles}\`;
        });
        const emptyState = document.querySelector('.empty-state');
        if (emptyState) emptyState.classList.toggle('hidden', hasVisibleContent);
    ...
    highlightNextBus() {
        ...
        allBubbles.forEach(bubble => {
            ...
            if (timeDiff >= 0 && timeDiff <= 60) {
                upcomingBubbles.push({ bubble, timeDiff, bubbleTotalMinutes });
            }
        });
    ...
    showBusDetails(time, busStop) {
        ...
        let timeMessage = '';
        if (timeDiff < 60) timeMessage = \`in \${timeDiff} min\`;
        else {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    generateSitemap() {
        ...
        urls.push({ loc: `${baseUrl}/`, lastmod: today, priority: '1.0', changefreq: 'daily' });
        ['about', 'contact', 'terms', 'privacy', 'disclaimer'].forEach(page => {
            urls.push({ loc: `${baseUrl}/${page}.html`, lastmod: today, priority: '0.8', changefreq: 'monthly' });
        });
        urls.push({ loc: `${baseUrl}/blogs/index.html`, lastmod: today, priority: '0.9', changefreq: 'weekly' });
        this.blogs.forEach(blog => {
            urls.push({ loc: `${baseUrl}/blogs/${blog.id}.html`, lastmod: new Date(blog.date).toISOString().split('T')[0], priority: '0.7', changefreq: 'monthly' });
        });
        Object.values(this.data.depots).forEach(depot => {
            const tehsil = this.data.tehsils[depot.tehsil_id];
            const district = tehsil ? this.data.districts[tehsil.district_id] : null;
            const division = district ? this.data.divisions[district.division_id] : null;
            if (division && district && tehsil) {
                urls.push({ loc: `${baseUrl}/${division.id}/${district.id}/${tehsil.id}/${depot.id}/index.html`, lastmod: today, priority: '0.6', changefreq: 'daily' });
            }
        });
    ...
    printStats() {
        ...
        if (fs.existsSync(this.publicImagesDir)) {
            const imageCount = fs.readdirSync(this.publicImagesDir).filter(f => /\.(jpg|jpeg|png|gif|webp|svg)$/i.test(f)).length;
        ...
        if (fs.existsSync(this.publicDir)) {
            ...
            faviconNames.forEach(name => {
                if (publicFiles.includes(name)) faviconFiles.push(name);
            });
        }
        console.log(`🌟 Favicon files in public root: ${faviconFiles.length > 0 ? faviconFiles.join(', ') : 'None found'}`);
        console.log('\n✅ All features work including cascading dropdowns on homepage.');
    }
