...
class SiteGenerator {
    constructor() {
        ...
        // NEW: Load custom head scripts from JSON file (if any)
        this.loadCustomHeadScripts();
    }

    // ==================== NEW METHOD: Load custom head scripts ====================
    loadCustomHeadScripts() {
        const scriptConfigPath = path.join(this.baseDir, 'head-scripts.json');
        if (fs.existsSync(scriptConfigPath)) {
            try {
                const scripts = JSON.parse(fs.readFileSync(scriptConfigPath, 'utf8'));
                if (Array.isArray(scripts)) {
                    this.customHeadScripts = scripts;
                    console.log(`📜 Loaded ${scripts.length} custom head script(s) from head-scripts.json`);
                } else {
                    console.warn('⚠️ head-scripts.json does not contain an array. No custom scripts added.');
                }
            } catch (err) {
                console.error('❌ Error loading head-scripts.json:', err.message);
            }
        } else {
            console.log('ℹ️ No head-scripts.json found – no custom scripts added.');
        }
    }

    // ==================== NEW METHOD: Programmatically add a script ====================
    addCustomHeadScript(scriptHtml) {
        if (typeof scriptHtml === 'string' && scriptHtml.trim()) {
            this.customHeadScripts.push(scriptHtml);
            console.log(`➕ Added custom head script (total: ${this.customHeadScripts.length})`);
        }
    }

    // ==================== MODIFIED: renderHeadScripts() now includes custom scripts ====================
    renderHeadScripts() {
        // Return all custom scripts concatenated
        return this.customHeadScripts.join('\n');
    }
